...
task("deploy-vaultrouter", "Deploy the JPEGVaultRouter contract").setAction(
    async ({}, { network, ethers, run, upgrades }) => {
        const JPEGVaultRouter = await ethers.getContractFactory(
            "JPEGVaultRouter"
        );
        ...
        if (network.name != "hardhat") {
            ...
            const impl = await (
                await upgrades.admin.getInstance()
            ).getProxyImplementation(config.jpegVaultRouter);
            ...
            try {
                ...
                await (
                    await vault.grantRole(
                        ...
                        "0x531277aa28cd919b8386b4c2013ed7f4df3b8a21"
                    )
                ).wait();
                ...
                await (
                    await vault.grantRole(
                        ...
                        "0xF9423E5cc3eE0956e7cB43BC7fffA9EA4C293F4d"
                    )
                ).wait();
                console.log(i, "done");
            } catch (error) {
...
task("update-providerImpl", "Upgrades the NFTVault contract").setAction(
    async ({}, hre) => {
        const { network, ethers, run, upgrades } = hre;
        const [deployer] = await ethers.getSigners();
        console.log("Deployer: ", deployer.address);
...
task("provider-upgrade", "Upgrades the NFTVault contract").setAction(
    async ({}, hre) => {
        ...
        const providerAddress = "0x7D68711F0f771C0A60E9d6c6B105efeaBABee903";
        const creditCap = [60, 100];
        const liqCap = [61, 100];
        const releaseDelay = 3600;
