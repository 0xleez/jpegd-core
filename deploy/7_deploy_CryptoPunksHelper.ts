...
task("deploy-punksHelper", "Deploys the CryptoPunksHelper contract").setAction(
    async (_, { network, ethers, run, upgrades }) => {
        ...
        const punksHelper = await upgrades.deployProxy(Helper, [
            config.cryptoPunks
        ]);

        console.log("CryptoPunksHelper deployed at: ", punksHelper.address);

        config.pusdAPunksHelper = punksHelper.address;
        ...
        if (network.name != "hardhat") {
            ...
            ).getProxyImplementation(punksHelper.address);
