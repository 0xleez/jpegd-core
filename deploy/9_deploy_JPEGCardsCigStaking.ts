...
task("deploy-cigStaking", "Deploys the JPEGCardsCigStaking contract")
    .addParam(
        "cigconfig",
        "A JSON file containing the contract's configuration",
        undefined,
        types.inputFile
    )
    .setAction(async ({ cigconfig }, { network, ethers, run }) => {
        ...
        const cigConfig = await JSON.parse(
            fs.readFileSync(cigconfig).toString()
        );
        ...
        const JPEGCardsCigStaking = await ethers.getContractFactory(
            "JPEGCardsCigStaking"
        );
        const cigStaking = await JPEGCardsCigStaking.deploy(
            config.jpegCards,
            cigConfig.cigarettes
        );
