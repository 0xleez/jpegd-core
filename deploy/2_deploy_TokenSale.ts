...
task("deploy-tokenSale", "Deploys the TokenSale contract").setAction(
    async (_, { network, ethers, run }) => {
        ...
        // if (!config.usdc) throw "No USDC address in network's config file";
        // if (!config.ethOracle)
        ...
        const TokenSale = await ethers.getContractFactory("TestERC721");
        const tokenSale = await TokenSale.deploy();
        await tokenSale.deployed();

        console.log("TokenSale deployed at: ", tokenSale.address);

        config.bayc = tokenSale.address;
